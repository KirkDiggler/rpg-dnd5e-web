...
/** Current first-look canvas extent; not a proposed backend/world limit. */
export const ROOM_AUTHORING_HEX_RADIUS = 6;
...
export function walkableCellsInWorldRectangle(
  ...
  for (
    let q = -ROOM_AUTHORING_HEX_RADIUS;
    q <= ROOM_AUTHORING_HEX_RADIUS;
    q += 1
  ) {
    for (
      let r = -ROOM_AUTHORING_HEX_RADIUS;
      r <= ROOM_AUTHORING_HEX_RADIUS;
      r += 1
    ) {
      if (
...
export function parseRoomDraftJson(json: string): RoomDraft {
  ...
  const envelope = JSON.parse(json) as Partial<RoomDraftEnvelope>;
  if (envelope.kind !== ROOM_DRAFT_KIND || envelope.version !== 1)
    throw new Error('Expected a version 1 room authoring draft.');
...
export function loadRoomDraft(
  ...
  try {
    const json = storage.getItem(ROOM_DRAFT_STORAGE_KEY);
    return { value: json ? parseRoomDraftJson(json) : fallback };
  } catch (error) {
    return {
      value: fallback,
