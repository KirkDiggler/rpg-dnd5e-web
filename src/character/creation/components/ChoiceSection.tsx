    ...
    <div className="mb-6">
      ...
      {choices.map((choice, index) => {
        ...
        if (!choice.from || !Array.isArray(choice.from)) {
          console.error('Invalid choice structure - missing or invalid "from" property:', choice);
          return (
            <div key={choiceKey} className="mb-4 p-3 rounded" style={{ backgroundColor: 'var(--bg-error)', color: 'var(--text-error)' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {choice.from.map((option) => {
                ...
                if (option.optionType === 'reference' && option.item) {
                  ...
                  const normalized = optionId.toLowerCase().replace(/^skill[:-]\s*/i, 'skill:');
                  const isAlreadyHave = existingSelections.has(optionId) || 
                                       existingSelections.has(normalized) ||
                                       existingSelections.has(option.item.name);
                  const isSelected = selected.includes(optionId);
                  const isDisabled = isAlreadyHave || (!isSelected && selected.length >= choice.choose);
                  ...
                  return (
                    <ChoiceCard
                      ...
                      description={isAlreadyHave ? 'Already have this proficiency' : `${choice.type} choice`}
                      selected={isSelected}
                      disabled={isDisabled}
                      onSelect={() => {
