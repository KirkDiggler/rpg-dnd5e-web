...
| `session-combat/`   | Shared-shell review harness | Deep link `?concept=session-combat` renders the production-owned `CombatExperience`, `ActionDock`, `TargetSurface`, `StoryLog`, and `DiceDrawer` with generated `Declaration`/`Participant`/`CharacterData` fixtures. It still reuses production `SessionCanvas` and collectible dice presentation; fixture controls and the contract inspector remain concept-only. No live stream/controller/RPC wiring yet.                                                                                                                                                                                                                                                                                     |
