...
| -------------------------------- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dnd5e.api.v1alpha2` (encounter) | `gen/ts/dnd5e/api/v1alpha2/encounter/*_pb` | `StreamEncounter`, all v2 event types, `TurnState`, `AvailableAction`, `InputRequired`, `Space` (`hexes`/`walls`/`entities`/`zones`), `Wall`/`WallKind`, `CharacterData` (`equipped`/`inventory`/`slots`/`armor_class_detail`/`main_hand_damage`, rpg-dnd5e-web#571) — the live game/harness stream |
| `dnd5e.api.v1alpha2` (character) | `gen/ts/dnd5e/api/v1alpha2/character/*_pb` | `CharacterService.EquipItem`/`UnequipItem` — character-scoped, out-of-encounter equip surface (rpg-dnd5e-web#571); returns the same `CharacterData` the encounter snapshot carries, no conversion layer                                                                                             |
...
**Current pin:** `github:KirkDiggler/rpg-api-protos#6363ebd6` (the `generated` branch) — the recover-the-artifact wave (rpg-project#368, slice 2 of the Living World journey). On the SESSION service it carries:

- `Loot(LootRequest{session, member, target, range})` and `Hold(HoldRequest{…})`. Both responses say only `saved`/`delivery` and **nothing about what moved** — an empty body and the captain answer with the same fields, which is the secret this slice keeps (design P3).
- Event kinds `LOOTED` / `HELD` / `DROPPED`, with typed bodies `Looted{looter, body}`, `Held{holder, prop}`, `Dropped{member, prop, at}`.
- `Exited.holding`, the prop ids carried out, and `Exited.exit`, the authored way out used — empty for a departure from anywhere the author did not name, which is the truth rather than "unknown".
- `AtlasProp.id`, the author's `place[].id`, empty when they named none, and `AtlasProp.holdable`, the author's flag verbatim. **The client offers Hold only where `holdable` is true and never infers it from an id** — ids exist for anything a scenario binds to, so guessing would put a Hold button on the altar as readily as on the reliquary (`src/components/session/holdingAffordances.ts`).
- `GetAtlasResponse.exits`, `repeated AtlasExit{id, at}` — the same for every member, since an exit is structure and not concealable. **For drawing the way out, never for gating it**: R9 requires a departure from anywhere to be possible, because that is the departure that drops the artifact where the carrier stood. So Leave is offered wherever the member stands, and the button only _names_ the exit when they are on one.

On the AUTHORING service: `ListScenarios` → `ScenarioDescriptor{id, name, fields[]}` with `ScenarioField{key, label, type, kind, guidance}` — the form the builder renders and derives nothing from (`src/author/ScenarioPanel.tsx`). `kind` is an open string on purpose; `FieldType` is a closed enum, because widget shapes are this client's vocabulary and bindable things are content.

**The pickup verb is Hold, not Take** (design R10): it writes a run-scoped `holds:` fact and nothing lands in a character's inventory, so `Taken` stays free for the merchant lane. The dungeon file's key is `holdable:`.

The pin before this one was `#883dd22`; `#46db48cd` was this same wave before the R10 rename.
The pin before this one was `#883dd22`. It carries `RegionRevealed.segments` and `RegionRevealed.sealed` (the reveal's patch: segments append, sealed replaces within the revealed region — `src/components/session/applyReveal.ts`), and `GetAtlasResponse.segments` (`repeated AtlasSegment`, a wall as the straight line its author drew, in fractional axial) and `GetAtlasResponse.sealed` (`repeated Position`, the cells in this recipient's atlas nobody can stand on) — rpg-project#360 slice 2. `AtlasProp.offsetX/offsetY` changed UNIT in the same wave, from circumradii to bounding-box fractions; the field did not move, only its meaning, so a stale reader is silently wrong rather than broken. Consumers pin the generated branch's commit, not a tag.
