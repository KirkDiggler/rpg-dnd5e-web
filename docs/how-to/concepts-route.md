...
| `world-building`          | Promoted shared editor (web#935/#951) | Deep link: `?concept=world-building` retains the local-only harness. The development main menu mounts the same editor as **World Builder** with a real `VITE_DEV_WORLD_ID` CompositionService source for immutable save/list/open. Local drafts, arrangements, import/export, continuous transforms, grouping/supports, gizmos, and authored visual lights remain shared. It does not change one-placement-per-cell dungeon YAML/API behavior. Contract: `src/concepts/world-building/CONTRACT.md`.                                                                                                                                                                                                                                                        |
