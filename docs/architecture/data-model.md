...
**Current pin:** `github:KirkDiggler/rpg-api-protos#46db48cd` (the `generated` branch), from rpg-api-protos#289 — the recover-the-artifact wave (rpg-project#368). It carries, on the SESSION service: `Loot(LootRequest{session, member, target, range})` and `Take(TakeRequest{…})`, whose responses say only `saved`/`delivery` and nothing about what moved; the event kinds `LOOTED`/`TAKEN`/`DROPPED` with typed bodies `Looted{looter, body}`, `Taken{taker, prop}`, `Dropped{member, prop, at}`; `Exited.holding` (the prop ids carried out) and `Exited.exit` (empty for a departure from anywhere the author did not name); and `AtlasProp.id`, the author's `place[].id`, empty when they named none. On the AUTHORING service: `ListScenarios` → `ScenarioDescriptor{id, name, fields[]}` with `ScenarioField{key, label, type, kind, guidance}` — the form the builder renders and derives nothing from (`src/author/ScenarioPanel.tsx`). **The verb is called `Take` on this pin and `Hold` everywhere else**: design R10 renamed it after #289 merged, because _take_ is reserved for a thing landing in inventory and this one only writes a run-scoped `holds:` fact. The wire renames in a wave-0 follow-up; until then the two words meet in exactly two files, `src/api/useSessionHold.ts` and `src/components/session/holdingBeat.ts`. The dungeon file's own key is already `holdable:`, which is what the toolkit's dungeonspec parses.
