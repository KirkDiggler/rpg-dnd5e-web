...
import type { GetDungeonResponse } from '@kirkdiggler/rpg-api-protos/gen/ts/dnd5e/api/authoring/v1alpha1/service_pb';
import { renderHook, waitFor } from '@testing-library/react';
import { beforeEach, describe, expect, it, vi } from 'vitest';

const hoisted = vi.hoisted(() => ({
  getDungeonFn: vi.fn<() => Promise<GetDungeonResponse>>(),
}));

vi.mock('@/api/client', () => ({
  authoringClient: { getDungeon: hoisted.getDungeonFn },
}));
...
beforeEach(() => {
  hoisted.getDungeonFn.mockReset();
  vi.resetModules();
});
...
describe('useAuthoringGate — probe classification', () => {
  ...
  it('a non-ConnectError throw (raw network failure) is also treated as unreachable', async () => {
    hoisted.getDungeonFn.mockRejectedValue(new TypeError('Failed to fetch'));
    const { useAuthoringGate } = await import('./useAuthoringGate');
  ...
  it('NotFound (service reachable, tomb absent) -> live', async () => {
    hoisted.getDungeonFn.mockRejectedValue(
      new ConnectError('no such dungeon', Code.NotFound)
    );
    const { useAuthoringGate } = await import('./useAuthoringGate');
  ...
  it('retry() re-probes and can recover from unreachable to live', async () => {
    hoisted.getDungeonFn.mockRejectedValueOnce(
      new ConnectError('failed to fetch', Code.Unavailable)
    );
    const { useAuthoringGate } = await import('./useAuthoringGate');
...
describe('useAuthoringGate — per-session caching', () => {
  ...
  it('does NOT cache unreachable — a second mount re-probes on its own', async () => {
    ...
    expect(hoisted.getDungeonFn).toHaveBeenCalledTimes(2);
  });
});
