...
`?concept=<id>` (for example `?concept=asset-anchor-lab`). There is no
...
| `asset-anchor-lab` | Learn verdict (web#731)     | Actual synced bookcase, ornate torch, and fighter standing/downed GLBs; exact-zero origin marker plus separately labeled visibility leader, raw/calibrated bounds, six facings, bounded resettable adjustment, one owning hex, and a positive load/measurement/render-acknowledged Orbit/Play gate. Non-production output only. Findings: `docs/evidence/asset-anchor-lab-731.md`. |
