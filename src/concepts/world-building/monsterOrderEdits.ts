/**
 * Editing ONE creature's own orders — its `on`, its `temper`, its `actions`
 * (rpg-dnd5e-web#1164).
 *
 ...
 * action a creature swings first, never orders the actions for the author, and
 * never reads a `when`. A weapon ref is carried exactly as typed — the engine
 * never interprets it — and whether a ref resolves is the server's judgement,
 * not this module's.
 *
 ...
 * REFUSES two states outright (`roomDraft.ts`'s `validateMonsterBindings`):
 *
 *   `actions: []`          -> "actions is empty; omit the key instead."
 *   a binding with no keys -> "declares no orders; omit the binding instead."
 *
 * So removing the last weapon is not a list edit — it must delete the
 * `actions` key, and if nothing else is left, the binding. An author who
 * emptied their last override would otherwise publish a document the engine
 * refuses. `normalizeBinding` is the ONE place that law lives, every helper
 * routes through it, and `withBinding` applies it again at the map level
 * exactly as `removeRoomMonster` does when a creature is deleted.
 *
