...
import { CharacterDraftContext, CharacterDraftState } from './CharacterDraftContextDef';

export function CharacterDraftProvider({ children }: { children: ReactNode }) {
  const [draft, setDraft] = useState<CharacterDraft | null>(null);
  const [allProficiencies, setAllProficiencies] = useState<Set<string>>(new Set());
  ...
  const collectRaceProficiencies = (race: RaceInfo | null): Set<string> => {
    ...
    if (race.proficiencies && Array.isArray(race.proficiencies)) {
      race.proficiencies.forEach(p => {
        ...
        const normalized = p.toLowerCase().replace(/^skill[:-]\s*/i, 'skill:');
    ...
    Object.values(raceChoices).flat().forEach(p => {
      proficiencies.add(p);
  ...
  const collectRaceLanguages = (race: RaceInfo | null): Set<string> => {
    ...
    Object.values(raceChoices).flat().forEach(l => {
  ...
  const setRace = useCallback((race: RaceInfo | null) => {
    // Update draft
    setDraft(prev => ({
      ...prev,
      race: race || undefined,
  ...
  const hasProficiency = useCallback((proficiencyIndex: string): boolean => {
    return allProficiencies.has(proficiencyIndex);
  }, [allProficiencies]);

  const hasLanguage = useCallback((languageIndex: string): boolean => {
  ...
  const addRaceChoice = useCallback((choiceKey: string, selection: string[]) => {
    setRaceChoices(prev => ({
      ...prev,
      [choiceKey]: selection,
    }));
  ...
  const addClassChoice = useCallback((choiceKey: string, selection: string[]) => {
