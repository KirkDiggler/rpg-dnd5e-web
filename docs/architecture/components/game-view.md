...
| Action menu    | `EncounterDock` composed from `ui/combat/*` (`components/game/`, `components/ui/combat/`) | The game-grade action surface (rpg-dnd5e-web#525; slice 1 PR #543, slice 2 PR #556 = the signed-off composition D). Renders verbatim off server-pushed `TurnState` — no client legality logic. HUD-sprite-skinned two-row bar: verbs INLINE with pool-shape cost badges from `economy_slot` (`ui/combat/organizeVerbs` + `inlineVerbs`; features as labeled groups, drop-down only under measured width pressure); no standing teaching strip — `ui/combat/ContextPill` floats guidance only for non-obvious states (armed / spectator #458 / free-roam #516 / ended / #545 nothing-left) with a hidden aria-live region announcing everything; the combat log renders OPEN by default as a click-through translucent panel over the map (hide persisted in `localStorage['ui.combatLog.hidden']`); reaction readiness lives behind the settings gear (left-anchored popover). `ActionMenu`/`EconomyBar` (`components/playtest/`) remain the `PlaytestHarness` dev-panel's surface, not the game path. |
