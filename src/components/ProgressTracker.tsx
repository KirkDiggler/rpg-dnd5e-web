...
export function ProgressTracker({
  ...
  return (
    ...
    >
      <ol className={cn(
        "flex",
        isHorizontal ? "flex-row items-center justify-between w-full" : "flex-col space-y-4"
      )}>
        {steps.map((step, index) => {
          const isClickable = step.status === 'completed' || (onStepClick && step.status !== 'disabled');
              ...
              <div className="flex items-center">
                <button
                  ...
                  className={cn(
                    ...
                    step.status === 'current' && "border-blue-600 bg-blue-600 text-white shadow-lg shadow-blue-600/25",
                    step.status === 'upcoming' && "border-gray-300 bg-white text-gray-500",
                    step.status === 'disabled' && "border-gray-200 bg-gray-100 text-gray-400",
